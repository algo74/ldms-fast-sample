...
#include "lustre_fulldump_osc_general.h"
...
static int sample(fulldump_sub_ctxt_p self)
{
  ...
  int err = oscs_refresh(&extra->source_tree, self, current_path);
  if (err) /* running out of set memory is an error */
    return err;

  oscs_sample(extra, osc_stats_sample);
  return 0;
}
...
int lustre_fulldump_osc_stats_config(fulldump_sub_ctxt_p self)
{
  ...
  self->term = (int (*)(void *self)) osc_general_term;
  return osc_extra_config(self, "stats");
}
