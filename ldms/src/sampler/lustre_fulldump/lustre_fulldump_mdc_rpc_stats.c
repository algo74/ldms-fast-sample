...
static const char *current_path = "/proc/fs/lustre/mdc";

/* Histogram schemas */
/* FIXME:
 * Regular histograms and metrics are likely always are zero, but we don't know for sure.
 * Modify histograms have only one value (not read and write as others),
 * but the schema has read and write only, so we store modify value in both.
 */
...
static struct ldms_metric_template_s hf_schema_templlate[] = {
    ...
    {"modify_RPCs_in_flight_n", 0, LDMS_V_U64, "", 1},
    {"modify_RPCs_in_flight_sum", 0, LDMS_V_U64, "", 1},
    {"modify_RPCs_in_flight_sum2", 0, LDMS_V_U64, "", 1},
    {0},
};
...
enum {
  ...
  MODIFY_RPC_IN_FLIGHT_N_ID,
  MODIFY_RPC_IN_FLIGHT_SUM_ID,
  MODIFY_RPC_IN_FLIGHT_SUM2_ID,
};
...
typedef struct hf_rpc_data {
  ...
  hf_rpc_single_metric_data_t modify_RPCs_in_flight;
} hf_rpc_data_t;

static int _read_one_metric_value_line(FILE *sf, const char *prefix, hf_rpc_single_metric_data_t *mdata, bool do_skip, const char *source_path)
{
  char buf[512];
  uint64_t val;
  int rc;
  // skip until line starts with prefix
...
static void hf_rpc_data_on_timer(struct server_data *server)
{
  ...
  // read "modify RPCs in flight"
  if (_read_one_metric_value_line(sf, "modify_RPCs_in_flight", &data->modify_RPCs_in_flight, false, source_path) != 0) {
    goto out1;
  }
  // read "read RPCs in flight"
  if (_read_one_metric_value_line(sf, "read RPCs in flight", &data->read_RPCs_in_flight, true, source_path) != 0) {
    goto out1;
  }
  // read "write RPCs in flight"
  if (_read_one_metric_value_line(sf, "write RPCs in flight", &data->write_RPCs_in_flight, false, source_path) != 0) {
    goto out1;
  }
  // read "pending write pages"
  if (_read_one_metric_value_line(sf, "pending write pages", &data->pending_write_pages, true, source_path) != 0) {
    goto out1;
  }
  // read "pending read pages"
  if (_read_one_metric_value_line(sf, "pending read pages", &data->pending_read_pages, true, source_path) != 0) {
    goto out1;
  }
...
static void hf_rpc_data_on_sample(struct server_data *server)
{
  ...
  ldms_metric_set_u64(metric_set, hf_schema_ids[MODIFY_RPC_IN_FLIGHT_N_ID], data->modify_RPCs_in_flight.count);
  ldms_metric_set_u64(metric_set, hf_schema_ids[MODIFY_RPC_IN_FLIGHT_SUM_ID], data->modify_RPCs_in_flight.sum);
  ldms_metric_set_u64(metric_set, hf_schema_ids[MODIFY_RPC_IN_FLIGHT_SUM2_ID], data->modify_RPCs_in_flight.sum2);
...
{
  return fulldump_general_schema_init(self, "lustre_fulldump_mdc_" SUB_SAMP, schema_templlate, schema_ids,
...
/**
 * \brief find and parse a part of the file that corresponds to modify RPCs in flight histogram
 * This is a special case hack because the histogram is not in the same format as the other histograms
 * and goes separately in front of the file
 * \return 0 on success, -1 if the end of file, errno on other error
 */
inline static int _parse_modify_histogram(char *buf, size_t buf_size, FILE *sf,
                                   ldms_set_t metric_set, ldms_mval_t handle, const char *source_path)
{
  char *metric_name = "rpcs in flight";
  int rc;
  ...
  while (fgets(buf, buf_size, sf)) {
    ...
    while (fgets(buf, buf_size, sf)) {
      ...
      // we parse as if it was a regular histogram but expect only two values
      rc = sscanf(buf, "%lu:%lu%*[^|]|%lu", &val1, &val2, &val3);
      if (rc != 2) {
        log_fn(LDMSD_LWARNING, SAMP " " SUB_SAMP "%s: failed to parse line in %s (modify %s): %s\n",
      ...
      snprintf(name_handle->a_char, MAXMETRICNAMESIZE, "modify_RPCs_in_flight");
      ldms_record_set_u64(rec_inst, schema_metric_record_ids[METRIC_BIN_ID], val1);
      // HACK: we use the same metric for both read and write because we don't have a column for modify
      ldms_record_set_u64(rec_inst, schema_metric_record_ids[METRIC_READ_COUNT_ID], val2);
      ldms_record_set_u64(rec_inst, schema_metric_record_ids[METRIC_WRITE_COUNT_ID], val2);
                           ...
                           ldms_set_t metric_set, ldms_mval_t handle, const char *source_path)
{
  ...
  while (fgets(buf, buf_size, sf)) {
    ...
    while (fgets(buf, buf_size, sf)) {
      ...
      rc = sscanf(buf, "%lu:%lu%*[^|]|%lu", &val1, &val2, &val3);
      if (rc != 3) {
        log_fn(LDMSD_LWARNING, SAMP " " SUB_SAMP "%s: failed to parse line in %s (%s): %s\n",
               __func__, source_path, metric_name, buf);
        return ENOMSG;
      }
      ldms_mval_t rec_inst = ldms_record_alloc(metric_set, schema_ids[METRIC_RECORD_ID]);
...
static int local_sample(const char *source_path, ldms_set_t metric_set)
{
  ...
  // process "modify"
  rc = _parse_modify_histogram(buf, sizeof(buf), sf, metric_set, handle, source_path);
  if (rc != 0) {
    err_code = rc == -1 ? 0 : rc;
    goto out2;
  }
...
static int sample(fulldump_sub_ctxt_p self)
{
  ...
  if (extra->hf_schema == NULL) {
    ...
    if (hf_schema_init(self, "lustre_fulldump_mdc_" SUB_SAMP "_hf", hf_schema_templlate, hf_schema_ids) < 0) {
...
int lustre_fulldump_mdc_rpc_stats_config(fulldump_sub_ctxt_p self)
{
